  ...
  return <section aria-labelledby="assistant-heading" className="mt-14 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:mt-16 sm:p-6">
    ...
    <div className="min-h-6" aria-live="polite" aria-busy={loading}>
      ...
      {result && <article className={`mt-5 overflow-hidden rounded-xl border border-slate-200 bg-slate-50 ${loading ? "opacity-70" : ""}`}>
        ...
        {result.sources.length > 0 && <div className="border-t border-slate-200 bg-white p-4 sm:px-5"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Sources</p><ul className="mt-2 flex flex-wrap gap-2">{result.sources.map((source) => <li key={source.id}><Link href={`/listing/${encodeURIComponent(source.id)}`} className="inline-flex min-h-10 items-center rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-blue-700 hover:border-blue-300 hover:bg-blue-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-700">{source.title}<span className="ml-1" aria-hidden="true">↗</span></Link></li>)}</ul></div>}
      </article>}
    </div>
  </section>;
}
