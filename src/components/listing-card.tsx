  ...
  return <article className={`group relative flex h-full min-w-0 flex-col overflow-hidden rounded-2xl border bg-white shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md focus-within:ring-2 focus-within:ring-blue-600 focus-within:ring-offset-2 ${compareSelected ? "border-blue-500 ring-1 ring-blue-500" : "border-slate-200 hover:border-blue-200"}`}>
    ...
    <div className="flex flex-1 flex-col p-4 sm:p-5">
      ...
      <div className="relative z-10 mt-auto flex items-center justify-between gap-3 pt-4">
        <Link href={href} className="inline-flex min-h-10 items-center gap-1 rounded-lg text-sm font-bold text-blue-700 underline decoration-blue-200 underline-offset-4 hover:decoration-blue-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-700">View details <span aria-hidden="true" className="transition-transform group-hover:translate-x-0.5">→</span></Link>
        {onToggleCompare && <button type="button" aria-pressed={compareSelected} aria-label={`${compareSelected ? "Remove" : "Add"} ${listing.title} ${compareSelected ? "from" : "to"} comparison`} onClick={() => onToggleCompare(listing.id)} className={`inline-flex min-h-10 items-center rounded-lg border px-3 text-sm font-bold transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-700 ${compareSelected ? "border-blue-700 bg-blue-700 text-white hover:bg-blue-800" : "border-slate-300 bg-white text-slate-700 hover:border-blue-400 hover:bg-blue-50 hover:text-blue-800"}`}>{compareSelected ? "✓ Selected" : "Compare"}</button>}
      </div>
    </div>
  </article>;
}
