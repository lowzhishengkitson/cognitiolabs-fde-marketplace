...
function Placeholder({ alt, decorative }: { alt: string; decorative: boolean }) {
  return <div role={decorative ? undefined : "img"} aria-hidden={decorative || undefined} aria-label={decorative ? undefined : `${alt} — image unavailable`} className="absolute inset-0 flex items-center justify-center bg-slate-100 text-slate-400">
...
export function ListingImage({ src, alt, sizes, priority = false, className = "", decorative = false }: Props) {
  const [failed, setFailed] = useState(false);
  const showImage = Boolean(src) && !failed;
  return <div className={`relative overflow-hidden bg-slate-100 ${className}`}>
    {!showImage && <Placeholder alt={alt} decorative={decorative} />}
