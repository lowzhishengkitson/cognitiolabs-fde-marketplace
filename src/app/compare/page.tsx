...
export default async function ComparePage({ searchParams }: Props) {
  ...
  if (resolved.listings.length !== 2) return <main className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-12">
    ...
    <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-9" aria-labelledby="compare-heading">
      ...
      {issues.length > 0 && <ul className="mt-5 space-y-1 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">{issues.map((issue) => <li key={issue} className="break-words">{issue}</li>)}</ul>}
    ...
    <section className="mt-8 grid gap-4 sm:grid-cols-2" aria-label="Compared laptops">
      {[first, second].map((listing) => <article key={listing.id} className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="p-5">
          ...
          <Link href={`/listing/${listing.id}`} className="mt-4 inline-flex min-h-11 items-center rounded-lg text-sm font-bold text-blue-700 underline decoration-blue-200 underline-offset-4 hover:decoration-blue-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-700">View full listing →</Link>
        </div>
      </article>)}
    </section>
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {row.values.map((value, index) => {
              ...
              return <div key={`${row.key}-${index}`} className={`min-w-0 rounded-lg border px-3 py-3 ${highlighted ? "border-emerald-200 bg-emerald-50" : "border-slate-100 bg-slate-50"}`}>
                <p className="break-words text-xs font-semibold text-slate-500">{resolved.listings[index].model}</p>
