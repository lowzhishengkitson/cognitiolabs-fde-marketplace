...
export function parseModelIntentResponse(payload: unknown): SearchIntent {
  if (typeof payload !== "object" || payload === null || !("choices" in payload)) throw new Error("Invalid response envelope");
  const choices = payload.choices;
  if (!Array.isArray(choices)) throw new Error("Invalid response choices");
