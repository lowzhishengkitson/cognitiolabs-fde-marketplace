...
function getClient(): OpenAI {
  ...
  if (!apiKey) throw new MissingEmbeddingKeyError("Embedding gateway is not configured");
...
async function embedTexts(texts: string[]): Promise<number[][]> {
  ...
  if (ordered.length !== texts.length || ordered.some((item, index) => item.index !== index)) throw new InvalidEmbeddingError("Incomplete embedding response");
