...
function getClient(): OpenAI {
  ...
  if (!apiKey) throw new Error("Embedding gateway is not configured");
...
async function embedTexts(texts: string[]): Promise<number[][]> {
  ...
  if (ordered.length !== texts.length || ordered.some((item, index) => item.index !== index)) throw new Error("Incomplete embedding response");
