...
export function parseLocally(query: string): SearchIntent 
{
  ...
  const sortField = namedSort ? (/cheap|expensive/.test(namedSort[0]) ? "price" : /battery/.test(namedSort[0]) ? "batteryHealth" : "weightKg")
  ...
  const direction = namedSort ? (/expensive|heaviest|highest|best/.test(namedSort[0]) ? "desc" : "asc") : sortPhrase ? (/desc/.test(sortPhrase[1] ?? sortPhrase[4] ?? "") ? "desc" : "asc") : undefined;
  const sort = sortField && direction ? { field: sortField, direction } : undefined;
