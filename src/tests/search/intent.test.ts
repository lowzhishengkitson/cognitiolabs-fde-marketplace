...
import { parseLocalQuery, parseLocally } from "../../lib/search/intent/local";

test("rejects malformed and contradictory structured intent", () => {
...
test("semantic query removes understood constraints but preserves open-ended meaning", () => {
  assert.equal(parseLocalQuery("RTX laptop under $1200 with at least 16GB RAM, cheapest first").semanticQuery, "laptop");
  assert.equal(parseLocalQuery("under $800 with at least 16GB RAM").semanticQuery, "laptop");
  assert.equal(parseLocalQuery("Lenovo good condition under $900").semanticQuery, "laptop");
  assert.equal(parseLocalQuery("RTX laptop for university").semanticQuery, "laptop for university");
  assert.equal(parseLocalQuery("something portable for university programming").semanticQuery, "something portable for university programming");
});
